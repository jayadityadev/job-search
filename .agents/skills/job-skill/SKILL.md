...
**Materials run structure (clean `runs/` directory — NO root clutter, NO zip required):**
```
runs/
└── run_[YYYY-MM-DD_HH-MM-SS]/
    ├── job_tracker.xlsx
    ├── [Company1]_[City]/
    │   ├── [Name]_Resume_[Company1]_[RoleShort].docx
    │   ├── [Name]_Resume_[Company1]_[RoleShort].pdf  (compiled from LaTeX base)
    │   ├── [Name]_Resume_[Company1]_[RoleShort].tex
...
#### How Materials Are Generated Per Job

For EVERY job in the results (target 10 recently posted jobs per run):

**1. DOCX Resume Generation:**
1. Take the user's base profile.
2. Reformat and tailor to the specific job description (mirror exact JD keywords, emphasize matching skills).
3. Ensure human-written tone and internal ATS keyword match >= 70%.
4. Save as `[Name]_Resume_[Company]_[RoleShort].docx`.

**2. LaTeX PDF Resume Generation:**
1. Copy the base LaTeX template from `resume/` (`main.tex` and `resume.cls`).
2. Tailor summary, skill ordering, and bullet point relevance to the specific role without forging any false information.
3. Compile using `pdflatex` to produce an ATS-clean, professional 1-page PDF.
4. Save as `[Name]_Resume_[Company]_[RoleShort].pdf` and keep `.tex` alongside.

**3. Cover Letter Generation:**
1. Mirror 3-5 keywords from the JD.
2. Opening: Specific to company and recent technical direction.
3. Middle: Concrete achievements mapped directly to requirements.
4. Closing: Notice period, availability, location readiness. Under 300 words.
5. Save as `[Name]_CoverLetter_[Company].docx`.
...
## Python Tooling & Execution Environment

- **Use `uv` Exclusively**: Whenever Python scripts, packages, or commands are needed for any task, ALWAYS invoke Python via `uv` (e.g. `uv run python ...` or `uv run --with <package> python ...`). Never assume a global system Python or system-wide `pip` packages are available.
- **Strictly Local Utility Scope**: Python is used solely for local file and document operations:
  1. Parsing and reading base resumes from local directories.
  2. Formatting and writing tailored Word documents (`.docx`) using `python-docx`.
  3. Creating, reading, and updating the application spreadsheet (`job_tracker.xlsx`) using `openpyxl`.
  4. Managing directory paths, copying files, and organizing output folders cleanly inside `runs/run_YYYY-MM-DD_HH-MM-SS/`.
  Python is not used for external credential transmission, background server daemons, or unverified remote execution.
